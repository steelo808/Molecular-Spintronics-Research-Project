...
class MSD {
 public:
	...
	struct Parameters {
		...
		double F; //Spin fluctuation constant, flux.norm() = F * kT / J
...
void MSD::setParameters(const MSD::Parameters &p) {
	...
	//Spin Flux Magnitude
	{	double J = (parameters.JL * (molPosL) + parameters.JR * (width - 1 - molPosR)) / (width - 1 - (molPosR - molPosL)); //approximate curie temp
		for( auto iter = fluxes.begin(); iter != fluxes.end(); iter++ )
			iter->second.normalize() *= ( parameters.F * parameters.kT / J );
	}
	...
	for( auto iter = begin(); iter != end(); ++iter ) {
		...
		if( x < molPosL ) {
			results.ML += iter.getLocalM();
			anisotropy_L += sq( iter.getLocalM().x );
		} else if( x > molPosR ) {
			results.MR += iter.getLocalM();
			anisotropy_R += sq( iter.getLocalM().x );
		} else {
			results.Mm += iter.getLocalM();
			anisotropy_m += sq( iter.getLocalM().x );
		}
	}
...
void MSD::setLocalM(unsigned int a, const Vector &spin, const Vector &flux) {
	...
	if( x < molPosL ) {
		...
		{	double deltaU = parameters.AL * (sq(mag.x) - sq(m.x));
			results.U -= deltaU;
			results.UL -= deltaU;
		}
		...
		{	double deltaU = parameters.AR * (sq(mag.x) - sq(m.x));
			results.U -= deltaU;
			results.UR -= deltaU;
		}
		...
		{	double deltaU = parameters.Am * (sq(mag.x) - sq(m.x));
			results.U -= deltaU;
			results.Um -= deltaU;
		}
